...
| BV2-03 | CSS/fonts/icons locales | BV2-02 | COMPLETA | `main` | `HEAD` (commit de esta tarea) | Tailwind v3.4.19 local (PostCSS+autoprefixer, plugins forms/container-queries) replicando config inline; `components/Icon.tsx` con 71 SVGs Material Symbols (Apache-2.0); fuentes Manrope+Noto Sans self-hosted woff2 (OFL) en `src/assets/fonts/` con font-display swap; index.html sin CDN (lang es-MX, favicon conservado); `npm run check` exit 0 (lint+typecheck+63/63 tests+build); CSS 4.35→79.05 kB min (1.49→13.62 kB gzip), JS 685.24→707.33 kB min (184.55→193.82 kB gzip); smoke 390×844 y 1440×900 con red bloqueada (solo Supabase/Meta Pixel intentan salir); cobertura de clases verificada (sin clases sin estilo salvo las que nunca existieron, ahora definidas) | LOCAL |
